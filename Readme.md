    ...
    4.02>OOBJclassB
        ...
        8.02>typeAssertion>structuralSubTyping
            -구조적서브타이핑이라는 타입호환 측면에서의 설명은:
                ...
                <!-- 명목타이핑 끝 -->
            9.01>generics>a
                -제네릭함수의 목적은 타입스크립트에서의 함수는 굉장히 제한된 타입에 대한 처리만 하고 재사용성이 감소하는 면모가 있으므로,
                ex)비밀번호는 문자, 생년월일은 숫자 라는 처리에서 변하지 않는다는 점.
                -그 지점에서 받는 매개변수의 타입을 유니언타입으로 확장시키는 등을 통해서 로직을 진행시키되, 타입에 대한 안정성도 적절히 가져갈 수 있도록 도와주는 장치이다.  
            9.02>generics>b
                -제네릭 인터페이스를 구현하는 제네릭 클래스를 설정할 수 있다,
                -일반적인 클래스와 달리 타입을 유니온타입을 통해서 2~3가지 타입에 따라 다양한 대응을 할 수 있는 클래스를 구현할 수 있다는 점에서: 타입체킹을 이전에 충족시켜주면서, 하나의 클래스의 타입을 늘려서, 다양한 상황을 커버할 수 있는 클래스를 구현할 수 있다는 것을 이야기 한다.
                -어렵게 볼일은 아니지만, 제대로 짜려면 20분정도는 설계에 대해 고민해야하는 부분이 있다. 마이크로서비스 상에서 하나의 클래스(프로토타입 메서드등)를 다양한타입의 인스턴스 객체가 공유한다는 것은 타입스크립트의 타입체킹상으로는 아직 안전한 일이지만, 어디까지 그 클래스에 의존할 것인지? 등을 비롯해서는, 성공적인 사례를 분석함으로써 장점을 체득, 이해하고 적용해야한다. 
    -------------boarder-------------line----------------------
